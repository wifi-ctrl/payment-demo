...
**Demo 实现**（聚焦支付核心，省略 Order/Fulfillment）：

```
[认证] → [浏览商品] → [选择商品] → [应用优惠券] → [Card/PayPal 支付]
  │          │            │            │                │
Identity  Catalog      Catalog      Coupon          Payment
                                                   (含定价计算)
→ [授权] → [扣款] → [退款]
    │        │        │
...
├── merchant/      商户管理 — 商户注册/暂停、渠道凭据（CARD/PAYPAL）
├── coupon/        优惠券   — 创建、核销（Apply/Rollback Saga 补偿）
├── payment/       支付核心 — Card/PayPal 购买 → 授权 → 扣款 → 退款（含定价计算）
...
> **参考架构**中还包含 Pricing（独立定价）、Order（订单流转）、Fulfillment（发货重试）三个上下文。
> Demo 简化处理：Pricing 合并为 `payment/domain/service/price_calculator.go`，Order 和 Fulfillment 属于扩展方向。
...
payment  ──ACL adapter────▶ catalog       adapter/catalog/ → CatalogQuery
payment  ──ACL adapter────▶ card          adapter/card/ → CardQuery
payment  ──ACL adapter────▶ merchant      adapter/merchant/ → MerchantQuery
payment  ──ACL adapter────▶ coupon        adapter/coupon/ → CouponApplier
payment  ──ACL (infra)────▶ Stripe        infra/stripe → StripeGatewayAdapter
payment  ──ACL (infra)────▶ PayPal        infra/paypal → PayPalGatewayAdapter
card     ──ACL (infra)────▶ Stripe        infra/stripe → StripeVaultAdapter
```
...
|---|---|---|---|
| **上下文** | Identity, Catalog, Pricing, Payment, Fulfillment, Order | Identity, Catalog, Card, Merchant, Coupon, Payment | Demo 增加了 Card/Merchant/Coupon 独立上下文，省略了 Order/Fulfillment |
| **Pricing** | 独立上下文：PriceTable + Discount 聚合 | 合并到 `payment/domain/service/price_calculator.go` | Demo 的定价逻辑简单（原价-折扣+税），不需要独立上下文 |
...
| **基础设施** | PostgreSQL + Redis + Kafka + Outbox | 全内存 + `infra/stripe` + `infra/paypal` | Demo 用内存仓储 + mock server，替换为真实实现只需换适配器 |
...
> **Demo 状态**：Demo 中定价逻辑已合并为 `payment/domain/service/price_calculator.go`（纯函数：原价-折扣+税）。
> 以下是定价规则复杂化后独立为上下文的参考设计。当定价需要区域定价、多折扣叠加、价格表管理时，应拆为独立上下文。
...
> - Demo 新增 **Merchant** 上下文实现多商户隔离（`GatewayFactory` 按凭据动态构建 Gateway）
> - Demo 新增 **Coupon** 上下文，UseCase 中有 Apply/Rollback Saga 补偿
> - Demo 的 `PaymentGateway` 接口带 `context.Context`，无 `idempotencyKey` 和 `Void`
> - Demo 通过 `infra/stripe` 和 `infra/paypal` 共享 HTTP 客户端，adapter 层做 ACL 翻译
> - 详细实现见 `README.md` 和 `internal/payment/` 源码
...
│   │   ├── domain/model/                         #   Merchant, ChannelCredential, errors
│   │   ├── domain/port/                          #   MerchantRepository
│   │   ├── domain/event/                         #   MerchantRegistered, CredentialAdded...
│   │   ├── application/                          #   MerchantUseCase
│   │   ├── adapter/persistence/                  #   InMemoryMerchantRepository
│   │   └── handler/http/                         #   MerchantHandler
│   │
...
│   │
│   ├── payment/                                  # ─── Payment Context（核心）───
│   │   ├── domain/
│   │   │   ├── model/                            #   PaymentTransaction, CardToken, PayPalToken, Money(alias), errors
│   │   │   ├── port/                             #   PaymentGateway, PayPalGateway, GatewayFactory,
│   │   │   │                                     #   TransactionRepository, CatalogQuery, CardQuery,
│   │   │   │                                     #   MerchantQuery, CouponApplier, TaxRateQuery
│   │   │   ├── event/                            #   PaymentAuthorized, PaymentCaptured, PaymentRefunded
│   │   │   └── service/                          #   CalculateFinalAmount（纯函数：原价-折扣+税）
│   │   ├── application/                          #   ChargeUseCase（Card + PayPal 购买/扣款/退款）
│   │   ├── adapter/
│   │   │   ├── gateway/                          #   StripeGatewayAdapter, PayPalGatewayAdapter,
│   │   │   │                                     #   MultiChannelGatewayFactory（ACL: infra → domain）
│   │   │   ├── persistence/                      #   InMemoryTransactionRepository
│   │   │   ├── catalog/                          #   CatalogAdapter（ACL: catalog → payment port）
│   │   │   ├── card/                             #   CardAdapter（ACL: card → payment port）
│   │   │   ├── merchant/                         #   MerchantAdapter（ACL: merchant → payment port）
│   │   │   ├── coupon/                           #   CouponAdapter（ACL: coupon → payment port）
│   │   │   └── tax/                              #   StaticTaxQuery（演示用静态税率）
...
│   └── infra/                                    # ─── 共享基础设施 ───
│       ├── config/                               #   环境变量加载
│       ├── stripe/                               #   Stripe HTTP 客户端（NewClient / NewMockClient）
│       │   ├── client.go                         #   typed 方法：CreateToken, CreatePaymentIntent...
│       │   └── mock_server.go                    #   内嵌 mock server（Demo 环境）
│       └── paypal/                               #   PayPal HTTP 客户端（NewClient / NewMockClient）
│           ├── client.go                         #   typed 方法：AuthorizeOrder, CapturePayment...
│           └── mock_server.go                    #   内嵌 mock server（Demo 环境）
```
...
| **infra/*** | 共享基础设施 | 外部 HTTP 客户端（stripe/paypal）、配置 | 标准库 |
| **bootstrap/** | Composition Root | 组装所有上下文（wire 函数） | 所有包 |
| **main.go** | 入口 | 加载配置 → 调用 bootstrap.New → 启动 HTTP | bootstrap, infra/config |
