// Package http 提供 merchant 上下文的 HTTP 驱动适配器。
package http
...
func (h *MerchantHandler) handleRegister(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(toMerchantResponse(m))
}
...
func (h *MerchantHandler) handleAddCredential(w http.ResponseWriter, r *http.Request) {
	...
	if req.Channel == "" {
		jsonError(w, "channel is required", http.StatusBadRequest)
		return
	}
	if len(req.Secrets) == 0 {
		jsonError(w, "secrets are required", http.StatusBadRequest)
		return
	}

	m, err := h.useCase.AddCredential(r.Context(), application.AddCredentialRequest{
		MerchantID: req.MerchantID,
		Channel:    model.PaymentChannel(req.Channel),
		Secrets:    req.Secrets,
	})
	if err != nil {
		jsonError(w, err.Error(), mapErrorStatus(err))
...
// mapErrorStatus merchant 上下文错误 → HTTP 状态码映射。
