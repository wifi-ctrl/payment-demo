...
func (m *Merchant) AddCredential(channel PaymentMethod, secrets map[string]string) error {
...
func (m *Merchant) ActiveCredential(channel PaymentMethod) (*ChannelCredential, error) {
	for i, c := range m.Credentials {
		if c.Channel == channel && c.Status == CredentialStatusActive {
