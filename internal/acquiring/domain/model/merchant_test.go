...
func TestMerchant_AddCredential_FirstTime_Success(t *testing.T) {
	...
	err := m.AddCredential(model.PaymentMethodCard, map[string]string{"api_key": "sk_live_xxx"})
	...
	if cred.Channel != model.PaymentMethodCard {
...
func TestMerchant_AddCredential_DuplicateActiveChannel_ReturnsError(t *testing.T) {
	...
	err := m.AddCredential(model.PaymentMethodCard, map[string]string{"api_key": "sk_live_new"})
...
func TestMerchant_AddCredential_AfterRevoke_Success(t *testing.T) {
	...
	for _, c := range m.Credentials {
		if c.Channel == model.PaymentMethodCard && c.Status == model.CredentialStatusActive {
			activeCount++
		}
	}
	if activeCount != 1 {
...
func TestMerchant_AddCredential_DifferentChannels_BothActive(t *testing.T) {
	...
	err := m.AddCredential(model.PaymentMethodPayPal, map[string]string{
		"client_id":     "xxx",
		"client_secret": "yyy",
	})
	...
	for _, c := range m.Credentials {
		...
		if c.Channel == model.PaymentMethodPayPal && c.Status == model.CredentialStatusActive {
			paypalActive = true
		}
	}
	if !cardActive {
...
func TestMerchant_ActiveCredential_Found_ReturnsCred(t *testing.T) {
	...
	cred, err := m.ActiveCredential(model.PaymentMethodCard)
...
func TestMerchant_ActiveCredential_RevokedOrMissing_ReturnsError(t *testing.T) {
	...
	// AC-15: PAYPAL 渠道不存在
	cred2, err2 := m.ActiveCredential(model.PaymentMethodPayPal)
...
func TestMerchant_ClearEvents_ReturnsAndClearsEvents(t *testing.T) {
	// AC-16
	m := activeMerchant()
	_ = m.AddCredential(model.PaymentMethodCard, map[string]string{"api_key": "sk"})
...
func TestMerchant_AddCredential_DeactivatedMerchant_ReturnsError(t *testing.T) {
	...
	err := m.AddCredential(model.PaymentMethodCard, map[string]string{"api_key": "sk"})
