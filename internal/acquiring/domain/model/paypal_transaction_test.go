...
func newPayPalTxn() *model.PaymentTransaction {
	return model.NewPayPalTransaction("user-1", "order-1", validMoney(), validPayPalToken())
}
...
func TestNewPayPalTransaction_Factory_CreatesWithCorrectFields(t *testing.T) {
	...
	txn := model.NewPayPalTransaction("user-42", "order-99", amount, token)
	after := time.Now()
	...
	if txn.OrderID != "order-99" {
		t.Errorf("OrderID: want order-99, got %s", txn.OrderID)
	}
}
...
func TestNewPayPalTransaction_EmptyToken_CreatesSuccessfully(t *testing.T) {
	...
	txn := model.NewPayPalTransaction("user-1", "order-1", validMoney(), emptyToken)
...
func TestNewPayPalTransaction_vs_NewPaymentTransaction_DifferentMethod(t *testing.T) {
	...
	cardTxn := model.NewPaymentTransaction("user-1", "order-1", validMoney(), cardToken)
	paypalTxn := model.NewPayPalTransaction("user-1", "order-1", validMoney(), validPayPalToken())
